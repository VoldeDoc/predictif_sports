...
const Gameweek: React.FC = () => {
  ...
  const { getFantasySquadPlayers, getMatchDay, setFormation, setSquad, getMatchDaySquad } = useDashBoardManagement();

  const [isSaving, setIsSaving] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [squadPlayers, setSquadPlayers] = useState<any[]>([]);
  const [fetchError, setFetchError] = useState<string | null>(null);
  const [matchdayData] = useState<any>(null);
  const [matchdayDataId, setMatchdayDataId] = useState<string | null>(null);
  // Add state to track if a squad already exists for this matchday
  const [squadAlreadyExists, setSquadAlreadyExists] = useState<boolean>(false);
  ...
  // Check if there is selected squad for the matchday 
  useEffect(() => {
    const checkMatchday = async () => {
      try {
        const matchdayResponse = await getMatchDay();
        if (matchdayResponse && Array.isArray(matchdayResponse) && matchdayResponse.length > 0) {
  ...
  // Check if there is already a squad for matchday so that a user won't be allowed to set squad again
  useEffect(() => {
    const checkMatchdaySquad = async () => {
      // Don't proceed if matchdayDataId is not available
      if (!matchdayDataId) {
        return;
      }
      
      try {
        const matchdaySquad = await getMatchDaySquad(String(matchdayDataId));
        
        if (matchdaySquad && 
            Array.isArray(matchdaySquad) && 
            matchdaySquad.length > 0 && 
            matchdaySquad[0]) {
          
          console.log("Matchday squad exists:", matchdaySquad[0].game);
          
          // Set state to indicate squad already exists
          setSquadAlreadyExists(true);
          
          // Notify the user
          toast.info("You've already selected a squad for this matchday");
        } else {
          setSquadAlreadyExists(false);
        }
      } catch (error) {
        console.error("Error fetching matchday squad data:", error);
      }
    };
  ...
  // Component to show when squad already exists
  const AlreadySubmittedView = () => {
    return (
      <div className="bg-white rounded-lg shadow-md p-8 text-center">
        <CheckCircle size={48} className="mx-auto mb-4 text-green-500" />
        <h3 className="text-xl font-bold mb-3">Squad Already Selected</h3>
        <p className="text-gray-600 mb-6">
          You've already selected your squad for this matchday. You can view the points for your selected players.
        </p>
        <button
          onClick={goToPoints}
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-md transition duration-200"
        >
          View Points
        </button>
      </div>
